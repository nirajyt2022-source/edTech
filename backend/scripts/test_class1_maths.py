#!/usr/bin/env python3
"""
Deterministic test for Class 1 Maths topic profiles (Phase 10A).

Validates 8 new topics — no LLM or API key needed.

Usage:
  cd backend
  python scripts/test_class1_maths.py
"""
...
# ── The 8 Class 1 Maths topics (canonical names) ──
CLASS1_TOPICS = [
...
TOPIC_ALIASES = {
    "Numbers 1 to 50 (Class 1)": [
        "class 1 numbers small", "c1 numbers small",
        "Numbers 1 to 50 (Class 1)",
    ],
    "Numbers 51 to 100 (Class 1)": [
        "class 1 numbers big", "c1 numbers big",
        "Numbers 51 to 100 (Class 1)",
    ],
    "Addition up to 20 (Class 1)": [
        "class 1 addition", "c1 addition",
        "Addition up to 20 (Class 1)",
    ],
    "Subtraction within 20 (Class 1)": [
        "class 1 subtraction", "c1 subtraction",
        "Subtraction within 20 (Class 1)",
    ],
    "Basic Shapes (Class 1)": [
        "class 1 shapes", "c1 shapes",
        "Basic Shapes (Class 1)",
    ],
    "Measurement (Class 1)": [
        "class 1 measurement", "c1 measurement",
        "Measurement (Class 1)",
    ],
    "Time (Class 1)": [
        "class 1 time", "c1 time",
        "Time (Class 1)",
    ],
    "Money (Class 1)": [
        "class 1 money", "c1 money",
        "Money (Class 1)",
    ],
}

# ── Topics where disallowed_keywords must include arithmetic cross-contamination ──
ADDITION_TOPICS = {"Addition up to 20 (Class 1)"}
SUBTRACTION_TOPICS = {"Subtraction within 20 (Class 1)"}

# ── Arithmetic canonical names (no _TOPIC_CONSTRAINTS entry needed) ──
...
print("=" * 60)
print("=== Phase 10A: Class 1 Maths \u2014 Deterministic Checks ===")
print("=" * 60)

for topic in CLASS1_TOPICS:
    print(f"\n{topic}:")
    ...
    if topic in ARITHMETIC_CANONICALS:
        # Arithmetic topics may or may not need constraints; check anyway
        if topic in _TOPIC_CONSTRAINTS:
            ok("Has constraints")
        else:
            # For Class 1 arithmetic, constraints are still expected to guard scope
            fail(f"Missing entry in _TOPIC_CONSTRAINTS")
    elif topic in _TOPIC_CONSTRAINTS:
        ok("Has constraints")
    else:
        fail(f"Missing entry in _TOPIC_CONSTRAINTS")
...
CROSS_CONTAM_ADD = {"multiply", "divide", "fraction"}
CROSS_CONTAM_SUB = {"multiply", "divide", "fraction"}
...
for topic in CLASS1_TOPICS:
    ...
    if topic in ADDITION_TOPICS:
        missing = CROSS_CONTAM_ADD - disallowed_lower
        if not missing:
            ok(f"{topic}: disallows multiply/divide/fraction")
        else:
            fail(f"{topic}: missing disallowed keywords: {missing}")

    if topic in SUBTRACTION_TOPICS:
        missing = CROSS_CONTAM_SUB - disallowed_lower
        if not missing:
            ok(f"{topic}: disallows multiply/divide/fraction")
        else:
            fail(f"{topic}: missing disallowed keywords: {missing}")

    # All Class 1 topics should disallow "decimal"
