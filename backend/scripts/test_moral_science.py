#!/usr/bin/env python3
"""
Deterministic test for Class 1-5 Moral Science topic profiles.

Validates 10 Moral Science topics — no LLM or API key needed.

Usage:
  cd backend
  python scripts/test_moral_science.py
"""
...
# ── The 10 Moral Science topics (canonical names) ──
CLASS1_MORAL_TOPICS = [
    "Sharing (Class 1)",
    "Honesty (Class 1)",
]
...
TOPIC_ALIASES = {
    ...
    "Global Citizenship (Class 5)": [
        "class 5 global citizenship",
        "cultural diversity",
        "Global Citizenship (Class 5)",
    ],
    "Digital Ethics (Class 5)": [
        "class 5 digital ethics",
        "online safety",
        "Digital Ethics (Class 5)",
    ],
}

# ── Maths keywords that Moral Science topics must disallow ──
MATHS_KEYWORDS = {"add", "subtract", "multiply", "divide"}


print("=" * 60)
print("=== Class 1-5 Moral Science \u2014 Deterministic Checks ===")
print("=" * 60)

for topic in ALL_MORAL_TOPICS:
    print(f"\n{topic}:")
    ...
    # ── 2. Has all required keys + subject = "Moral Science" ──
    missing_keys = REQUIRED_KEYS - set(profile.keys())
    if not missing_keys:
        if profile.get("subject") == "Moral Science":
            ok("Has required keys + subject='Moral Science'")
        else:
            fail(f"subject='{profile.get('subject')}', expected 'Moral Science'")
    else:
...
print(f"\n{'=' * 60}")
print("Grade-specific checks: subject='Moral Science' + disallow maths keywords:")
print("=" * 60)
...
for topic in ALL_MORAL_TOPICS:
    ...
    # All Moral Science topics must have subject = "Moral Science"
    if profile.get("subject") == "Moral Science":
        ok(f"{topic}: subject='Moral Science'")
    else:
        fail(f"{topic}: subject='{profile.get('subject')}', expected 'Moral Science'")

    # All Moral Science topics must disallow maths keywords
