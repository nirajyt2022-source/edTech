#!/usr/bin/env python3
"""
Deterministic test for Class 1 English topic profiles.

Validates 7 new topics — no LLM or API key needed.

Usage:
  cd backend
  python scripts/test_class1_english.py
"""
...
# ── The 7 Class 1 English topics (canonical names) ──
CLASS1_TOPICS = [
    "Alphabet (Class 1)",
    "Phonics (Class 1)",
...
TOPIC_ALIASES = {
    "Alphabet (Class 1)": [
        "class 1 alphabet", "c1 alphabet",
        "Alphabet (Class 1)",
    ],
    "Phonics (Class 1)": [
        "class 1 phonics", "c1 phonics",
        "Phonics (Class 1)",
    ],
    "Self and Family Vocabulary (Class 1)": [
        "class 1 family", "c1 family", "family vocabulary",
        "Self and Family Vocabulary (Class 1)",
    ],
    "Animals and Food Vocabulary (Class 1)": [
        "class 1 animals", "c1 animals", "animals and food",
        "Animals and Food Vocabulary (Class 1)",
    ],
    "Greetings and Polite Words (Class 1)": [
        "class 1 greetings", "c1 greetings",
        "Greetings and Polite Words (Class 1)",
    ],
    "Seasons (Class 1)": [
        "class 1 seasons", "c1 seasons",
        "Seasons (Class 1)",
    ],
    "Simple Sentences (Class 1)": [
        "class 1 sentences", "c1 sentences", "simple sentences",
        "Simple Sentences (Class 1)",
    ],
}
...
print("=" * 60)
print("=== Class 1 English — Deterministic Checks ===")
print("=" * 60)
...
for topic in CLASS1_TOPICS:
    ...
    # All Class 1 English topics must have subject = "English"
    if profile.get("subject") == "English":
        ok(f"{topic}: subject='English'")
    else:
        fail(f"{topic}: subject='{profile.get('subject')}', expected 'English'")

    # All Class 1 English topics must disallow maths keywords
